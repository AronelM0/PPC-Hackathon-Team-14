#!/usr/bin/env python3
"""
knn_series_predictor.py

Advanced forecaster + Profit-Aware Trading.

Models supported:
  - RandomForest (rf)
  - GradientBoosting (gbm, via MultiOutputRegressor)
  - RF+GBM ensemble (ensemble)
  - K-Nearest Neighbors (knn)  <-- NEW, non-tree, non-GBM
...
Trading:
  - Profit-aware: trade only on cheapest / most expensive slots,
    with min spread and daily SoC reset + end-of-day flush.
"""
...
def generate_battery_actions(
    ...
    This uses only forecast RANK, not absolute level, and will usually
    generate **lots** of trades on volatile days.
    """
    ...
    for day in range(n_days_eff):
        ...
        # Compute quantile thresholds for this day
...
def parse_args():
    ...
    parser.add_argument("--out_csv", type=str, default="energy-trading-hackathon-2025/submission_knn_v1.csv")
...
def main():
    ...
    lags = [1, 2, 3, 4, 96, 96 * 2, 96 * 7, 96 * 28]  # 15min, 1h, 1d, 2d, 1w style, 1m style
    ...
    if args.model_type in ("gbm", "ensemble"):
        base_gbm = GradientBoostingRegressor(
            ...
            max_depth=args.gm_max_depth if hasattr(args, "gm_max_depth") else args.gbm_max_depth,
